...
CLI·Chrome 없이 쓰는 경로는 `gem/README.md`에 있습니다. `gem/worksheet-render.html` 한 파일이
실측 조판·학생용 정답 물리 제거·A4 인쇄를 수행하고, 활동지 JSON은 두 곳에서 옵니다.

- 앱 내장 AI 패널 — Gemini API 또는 OpenRouter를 브라우저에서 직접 호출(키는 localStorage).
  성취기준은 내장 대장에서 추려 프롬프트에 실으므로 모델이 코드를 창작하지 못합니다.
- 챗봇 복붙 — Gem·GPT·Claude 프로젝트가 만든 JSON을 붙여넣기.

저작 규격 정본은 `gem/knowledge/worksheet-json-spec.md`, 시스템 프롬프트는
`tools/ai-system-prompt.md`입니다. 재빌드는 `npm run build:gem`.
